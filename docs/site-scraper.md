...
This was the initial idea, which is still baked into the routes file.
...
The first supported scraped site is nakedcapitalism.com (WordPress REST API), which is
classified as long-form posts or links-roundups and surfaced with a deliberate
~4-month reading lag.

When start up completes, try:
